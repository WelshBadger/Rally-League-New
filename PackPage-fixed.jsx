...
function PinBadge({ type, value }) {
  ...
  const labels = { postcode: '📍', w3w: '///', apple: '🗺' }
  const colors = { postcode: '#06b6d4', w3w: '#e11d48', apple: '#3b82f6' }
  return (
    <a
      href={url}
      target="_blank"
      rel="noopener noreferrer"
      className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-medium no-underline transition-all hover:opacity-80"
      style={{ background: colors[type] + '20', color: colors[type], border: `1px solid ${colors[type]}40` }}
    >
      <span>{labels[type]}</span>
      <span>{type === 'w3w' ? '///' + value.replace(/^\/\/\//, '') : type === 'postcode' ? value.toUpperCase() : 'Open in Maps'}</span>
...
function LocationCard({ label, locData, onChange }) {
  const data = typeof locData === 'object' && locData !== null ? locData : { text: locData || '', pinType: null, pin: '' }
  const [pinType, setPinType] = useState(data.pinType || null)
  ...
  function selectPinType(type) {
    const next = type === pinType ? null : type
    setPinType(next)
    onChange({ ...data, pinType: next, pin: next ? (data.pin || '') : '' })
  }

  return (
    <div className="bg-rl-card border border-white/10 rounded-xl px-4 py-3.5 space-y-3">
      ...
      <div className="flex gap-1.5 flex-wrap">
        {PIN_TYPES.map(pt => (
          <button
            key={pt.id}
            onClick={() => selectPinType(pt.id)}
            className="px-2.5 py-1 rounded-lg text-xs font-medium transition-all border"
            style={pinType === pt.id
              ? { background: '#06b6d420', color: '#06b6d4', borderColor: '#06b6d440' }
      ...
      {pinType && (
        <div className="space-y-2">
          <input
            ...
            placeholder={PIN_TYPES.find(p => p.id === pinType)?.placeholder}
            className="rl-input text-sm w-full"
          />
          {data.pin && <PinBadge type={pinType} value={data.pin} />}
        </div>
      )}
    </div>
  )
      ...
      <div>
        ...
        <div className="space-y-3">
          ...
          <div className="bg-rl-card border border-white/10 rounded-xl px-4 py-3">
            ...
            <textarea
              value={typeof locs.notes === 'string' ? locs.notes : ''}
              onChange={e => set('notes', e.target.value)}
              placeholder="Parking, access codes, anything else the team needs…"
              rows={3}
